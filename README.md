...
| **カレンダー連携**           | `.ics` ファイルのインポートと差分同期 / Google カレンダー iCal 同期 |
...
| **カレンダー**   | `/calendar` | .ics ファイルのインポートと同期状態 / Google カレンダー iCal 同期 |
...
カレンダーページの「Google Calendar」タブに Google カレンダーをそのまま表示できます。Google アカウントにログイン済みであれば埋め込み内で予定を閲覧可能です。

![Google Calendar 埋め込み表示](docs/images/google-calendar-embed.png)

**埋め込み URL の取得手順:**

1. [Google カレンダー](https://calendar.google.com) を開き、右上の歯車アイコン → 「設定」
...
![設定 - Google Calendar 連携](docs/images/google-calendar-settings.png)

### iCal 同期

Google カレンダーの iCal URL を使って同期する場合、**カレンダーの共有設定によってイベント名（タイトル）が取得できるかどうかが変わります。**

| URL の種類       | 取得できる情報                   |
| ---------------- | -------------------------------- |
| **公開 URL**     | 「予定あり」のみ（タイトルなし） |
| **限定公開 URL** | イベント名・詳細が取得可能       |

**限定公開 URL の取得手順:**

1. [Google カレンダー](https://calendar.google.com) を開く
2. 同期したいカレンダーの「⋮」→「設定と共有」をクリック
3. 「他のカレンダーとの統合」セクションまでスクロール
4. **「非公開の iCal 形式 URL」**をコピーする

> **注意:** 限定公開 URL は認証なしにカレンダーの全イベントにアクセスできます。第三者に共有しないでください。
